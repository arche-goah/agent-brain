...
- **The scheduled brain-scan no longer starts on battery.** Measured 2026-09-23 on a macOS brain: launchd fired the due scan at 09:12 on a lid-closed MacBook on battery. The machine never fully woke — `pmset -g log` shows only ~45 s dark wakes every 8-17 min — so every agent of the run froze with each sleep, and the workflow counted each freeze as a stall and restarted the agent (stalls of 1026/1050/1505 s, the length of the sleep gaps; one agent up to its fifth attempt). After the 90 min ceiling the run was killed: 2.34M tokens, no report. New `helpers/power-source.sh` prints `ac`, `battery` or `unknown` on macOS (`pmset -g batt`), Linux (`/sys/class/power_supply`) and Windows under Git Bash (`Win32_Battery.BatteryStatus` via PowerShell); a machine without a battery reports `ac`, and only `battery` means skip — `unknown` runs, so a desktop, a VM or a CI runner never loses its scheduled scan to a detector that cannot see a battery. `scripts/brain-scan.sh` asks it after the freshness gate: on battery it writes one skip line to `run.log` and exits 0 without a run-record `fail` (nothing failed; the scan stays due and fires on the next scheduled start on mains, and the bootup's report age still shows it overdue). `BRAIN_SCAN_FORCE=1` bypasses both gates, as before. Fixtures in `portability-smoke.sh`, on all three CI runners: a due instance on battery logs the skip and starts no run; the same instance on mains starts one (negative control — a gate that always skips would pass the first check alone); and the detector itself runs its real reader and answers with one of its three words. The macOS battery branch was additionally driven with a stubbed `pmset` reporting `'Battery Power'` (answer `battery`). Not yet measured: the Windows reader on a laptop on battery — the CI runner has no battery and only exercises the no-battery branch. Class searched: the only other headless `claude -p` caller in the proving instance (`rig-check.sh`) is not scheduled.
