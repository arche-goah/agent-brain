...
5. ${REPO}/.claude/skills/REGISTRY.md → corpus/skills/ (instance registry; may be missing/empty) PLUS the SKILL.md of these behavior-shaping skills (each as <name>.md) — they live under ${REPO}/core/skills/<name>/SKILL.md, instance skills under ${REPO}/.claude/skills/<name>/SKILL.md: session-close, caveman, ponytail, brain-scan workflow (${REPO}/core/workflows/brain-scan.js as brain-scan-workflow.js.txt), memory-dream, coherence-scan — PLUS every skill the instance's auto-fire table (${REPO}/.claude/rules/*.md) marks as behavior-shaping; that list is instance knowledge, read it, never assume it. A skill listed here but existing nowhere is an anomaly for the MANIFEST, not an abort.
